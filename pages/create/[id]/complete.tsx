...
export const getServerSideProps = async (context: any) => {
  ...
  const res = await adminFirestore
    ...
    .then(async (data) => {
      // Step 2: fetch the image from Firebase storage
      const res = await getDownloadURL(ref(clientStorage, data!.club_image))
        .then((link) => {
...
const StepComplete: NextPageWithLayout<any> = (serverProps: InferGetServerSidePropsType<typeof getServerSideProps>) => {
    ...
    <div className="flex flex-row items-center justify-center h-full w-full px-6">
      <div className="flex flex-col items-center justify-center gap-4 max-w-[480px]">
        <div className=" w-16 md:w-24 h-16 md:h-24 relative rounded-10 border-2 border-secondary-300 overflow-hidden">
          <Image src={serverProps.profileImgUrl} alt={`Profile image for ${serverProps.clubName}`} fill/>
        </div>
