...
export async function createSignClient(relayerRegionURL = process.env.NEXT_PUBLIC_WC_RELAY_URL) {
...
export function createLegacySignClient({ uri, clubWallet, setLegacySession }: { uri?: string, clubWallet:IClubWallet, setLegacySession:(session: any) => void }) {
  ...
  legacySignClient.on('session_request', (error, payload) => {
    ...
    ModalStore.open('LegacySessionProposalModal', { legacyProposal: payload }, clubWallet)
  })
...
export async function legacyKillSession () {
